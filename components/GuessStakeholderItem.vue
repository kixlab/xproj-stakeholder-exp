<template>
  <v-layout row wrap>
    ...
    <v-flex xs12 class="item__question__box">
      ...
      <v-radio-group v-model="isBenefit" row class="item__question__radio">
        <v-radio label="긍정적" :value="1"></v-radio>
        <v-radio label="부정적" :value="0"></v-radio>
      </v-radio-group>
    </v-flex>
    <!-- <v-flex v-if="seeAnswer" xs12 md5 :offset-md3="seeAnswer ? false : true" :key="`slider`">
      <v-slider
        max="7"
        min="1"
    ...
    <v-flex v-if="seeAnswer && selectedTag" xs12>
...
export default {
  ...
  methods: {
    onInput (ev) {
      this.selectedTag = this.numToHangul(ev)
      // this.$ga.event({
    ...
    onSeeMoreClick: function () {
      this.$emit('tag-see-more', this.selectedTag)
