  ...
  <v-layout row wrap>
    ...
    <v-flex xs12>
      <v-card color="grey lighten-4">
        <v-card-text>
          <span style="color: blue;">{{selectedTag}}</span>에게 이 정책은 어떤 영향을 줄까요?
        </v-card-text>
      </v-card>
    </v-flex>
    <v-tabs grow v-model="tab">
      <v-tab v-for="i in randomEffects.length" :key="i">
        {{randomEffects[i-1].stakeholder_detail}}
      </v-tab>
        ...
        <v-layout row wrap>
          <v-flex xs12>
            <p class="question">
              ...
              이 정책은 <strong>{{randomEffects[tab].stakeholder_detail}}</strong>의 삶을 어떻게 바꿀까요? 
              <strong>{{randomEffects[tab].stakeholder_detail}}</strong>의 입장에서 이야기해주세요.
              <v-flex xs10 style="text-align: left;">
                <font size="2"><span style="text-align:left; color:blue;" v-for="tag in randomEffects[tab].tags" :key="tag">#{{tag}}&nbsp;&nbsp;</span></font>
              </v-flex>
            </p>
          ...
          <v-layout row wrap>
            ...
            <v-flex xs8 sm8>
              <div>
                ...
                <v-btn 
                :outline="predictedEffects[tab].isBenefit !== 1" 
                :dark="predictedEffects[tab].isBenefit == 1"
                color="primary"
                class="binarybtn"
                @click="predictedEffects[tab].isBenefit=1">
                긍정적 </v-btn>
                <v-btn 
                :outline="predictedEffects[tab].isBenefit !== 0" 
                :dark="predictedEffects[tab].isBenefit == 0"
                color="error" 
                class="binarybtn" 
                @click="predictedEffects[tab].isBenefit=0"> 
                부정적 </v-btn>
              </div>
            </v-flex>
          </v-layout>
...
export default {
  ...
  computed: {
    ...
    randomEffect: function () {
      return this.$store.state.randomEffects[0]
    },
  ...
  //     })
  //     if (ev) {
  //       this.predictedEffects.source = 'stakeholder'
  //     } else {
  ...
  // },
  created: function () {
    this.predictedEffects = this.randomEffects.map((e) => {
      return {
        isBenefit: -1,
        stakeholder_detail: e.stakeholder_detail,
        stakeholder_group: 1,
        description: '',
        source: 'guess',
        confidence: 0,
