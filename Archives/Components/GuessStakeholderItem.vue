<template>
  <!-- <v-layout row wrap justify-center> -->
    <v-card>
      ...
      <v-card-text class="item__question__box">
        <span class="question"> 어떤 영향을 받을까요? </span>
        <v-radio-group v-model="guessedItem.isBenefit" :disabled="fixed || seeAnswer" row class="item__question__radio">
          <v-radio label="긍정적" :value="1"></v-radio>
          <v-radio label="부정적" :value="0"></v-radio>
        </v-radio-group>
        <span v-if="(fixed || seeAnswer) && guessedItem.tag">긍정적 {{selectedTagInfo ? selectedTagInfo.pos_count : ''}} vs {{selectedTagInfo ? selectedTagInfo.neg_count : ''}} 부정적</span>
      </v-card-text>
      <v-card-actions v-if="(fixed || seeAnswer) && guessedItem.tag"> 
        <v-btn block @click="onSeeMoreClick">
          더 알아보기
        </v-btn>
      </v-card-actions>
    </v-card>
...
export default {
  ...
  props: {
    ...
    guessedItem: {
      validator: function (obj) {
        return 'tag' in obj && 'isBenefit' in obj
      }
    },
    fixed: {
      default: false,
  ...
  computed: {
    selectedTagInfo: function () {
      return this.tags.find((t) => {
        return t.tag === this.guessedItem.tag
      })
    },
    tags: function () {
  ...
  data: function () {
    return {
      ...
      search: '',
      guessStance: 4,
      sliderLabels: ['매우 부정적', '', '', '', '', '', '매우 긍정적'],
      isBenefit: -1
    }
  },
  methods: {
    onInput (ev) {
      this.guessedItem.tag = this.numToHangul(ev)
      // this.$ga.event({
    ...
    onSeeMoreClick: function () {
      this.$emit('tag-see-more', this.guessedItem.tag)
